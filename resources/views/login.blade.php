...
<head>
  ...
  <link href="{{asset('/public/dashboard/login/css3/mainar.css')}}" rel="stylesheet" type="text/css"/>
    ...
    <body style="background-image: url('http://localhost:82/test/public/dashboard/login/img/desk.jpg');">
			...
			<div class="wrap-login100">
			 ...
			 @csrf
					...
					<div class="wrap-input100 validate-input" data-validate = "أدخل أسم المستخدم">
						...
						<input class="input100 form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" id="email" placeholder="أدخل أسم المستخدم" type="email" name="email" value="{{ old('email') }}" required autofocus >
                          @if ($errors->has('email'))
					...
					<div class="wrap-input100 validate-input" data-validate="أدخل كلمة السر">
						<input class="input100 form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" id="password" type="password" placeholder="أدخل كلمة السر" name="password" required>
